    (function () {
      ...
      /* ============ Masks (telefone + CNPJ) ============ */
      // Campo Ã© celular Â· sempre formato (XX) XXXXX-XXXX, traÃ§o sÃ³ aparece quando tem dÃ­gito depois
      ...
      if (form) {
        ...
        // Autofill do Chrome nÃ£o dispara `input` Â· usa animationstart do :-webkit-autofill como detector + change como backup
        ...
        form.addEventListener('submit', (e) => {
          e.preventDefault();
          // Re-aplica mÃ¡scaras antes de validar Â· cobre autofill (que nÃ£o dispara `input`)
      ...
      /* ============ Bloco 4 Â· Card-deck 3D (padrÃ£o patternbreak) ============ */
      ...
      /* ============ Timeline (Bloco 3 Â· scroll-driven, padrÃ£o Ciridae) ============ */
      ...
      if (tlTrack && !reduced) {
        ...
        /* Active item Â· IntersectionObserver com rootMargin centralizado */
      ...
      if (problemaTrack && !reduced) {
        ...
        function updateVxCards() {
          ...
          // Cards comeÃ§am quando panel trava Â· LOCK_OFFSET = quanto do track scrolla antes do lock
          // (intro section + padding) Â· ajusta esse valor pra alinhar progress=0 com o momento da trava
          ...
          // === Cards Â· CASCADE CONTÃNUO ===
          // Cada card tem motion contÃ­nua (entrada â†’ meio â†’ saÃ­da sem pausa).
          // Cards staggered Â· 01 entra, logo depois 02, 03, 04.
          // Quando 02 estÃ¡ entrando, 01 continua subindo atÃ© sumir.
          // No meio do bloco 2, vÃ¡rios cards visÃ­veis ao mesmo tempo em
          // diferentes posiÃ§Ãµes verticais (cascade flow).
          const RISE_FROM = window.innerHeight * 1.0;     // offscreen abaixo
          const RISE_TO = -window.innerHeight * 0.9;      // offscreen acima
          const CARD_STARTS = [0.00, 0.04, 0.08, 0.12];   // cards comeÃ§am imediatamente quando bloco 2 trava
          const CARD_DURATION = 0.38;                      // cada card tem ~38% do scroll
          const FADE_THRESHOLD = 0.55;                     // a partir desse t, comeÃ§a fade
          ...
          vxCards.forEach((card, i) => {
            ...
            if (progress < start) {
              ...
              // Motion contÃ­nua de RISE_FROM atÃ© RISE_TO
              const t = (progress - start) / CARD_DURATION; // 0 â†’ 1
              // Easing sine ease-in-out Â· movimento natural e fluido
              const tyEase = 0.5 - 0.5 * Math.cos(t * Math.PI);
              translateY = RISE_FROM + (RISE_TO - RISE_FROM) * tyEase;

              // Opacity Â· full opacity atÃ© FADE_THRESHOLD, depois fade out
              if (t <= FADE_THRESHOLD) {
                opacity = 1; // SEM fade in Â· card visÃ­vel desde que entra
              } else {
          ...
          // === Close text fade-in (cards jÃ¡ fora) â€” ainda dentro do lock ===
          if (closeEl) {
            let closeOpacity = 0;
          ...
          // === Insight bubble Â· entrada moderna depois dos cards passarem ===
          if (problemaInfo) {
            const REVEAL_AT = 0.55; // apÃ³s cards comeÃ§arem a fade out
        ...
        // reduced motion: cards visÃ­veis na posiÃ§Ã£o final, sem scroll-driven
      ...
      if (newsBubble && newsBubbleClose) {
        ...
        /* Abre depois de 1s Â· ancorado no hero, rola junto com a pÃ¡gina */
        setTimeout(() => {
          if (dismissed) return;
        ...
        /* X Â· dismiss permanente Â· sÃ³ some quando o usuÃ¡rio fechar */
